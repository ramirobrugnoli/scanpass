...
export default function BatchPassportScanner() {
  ...
  // Export to Excel (new functionality)
  const exportToExcel = useCallback(() => {
    // Get all completed and processed scans
    const processedScans = files
      .filter((f) => f.status === "completed" && f.processedResult)
      .map((f) => f.processedResult as StandardizedData);
    ...
    // Use our Excel export utility
    const excelFile = generateExcelFileFromMultiple(processedScans);
